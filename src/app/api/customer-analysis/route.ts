    ...
    if (analysisMode === 'web+erp') {
      ...
      const webResponse = await fetch('https://api.openai.com/v1/responses', {
        ...
        body: JSON.stringify({
          ...
          instructions: 'Bạn là chuyên gia CRM và chiến lược marketing của GUSA tại Việt Nam. Trả lời hoàn toàn bằng tiếng Việt. ERP là nguồn sự thật về khách hàng GUSA; web chỉ cung cấp bối cảnh thị trường. Bắt buộc nghiên cứu nhiều nguồn web độc lập, ưu tiên nguồn mới và đáng tin cậy. Chủ động lựa chọn số lượng số liệu ERP, khách hàng, phân khúc và tín hiệu thị trường cần thiết để lập chiến lược phù hợp; không áp đặt số lượng trích dẫn cố định. Mỗi đề xuất phải có căn cứ dữ liệu rõ ràng, nhưng chỉ dùng các số liệu thực sự liên quan. Nếu không đủ dữ liệu phải nói rõ. Không đưa lời khuyên chung chung, không bịa số và không lấy số web thay cho số ERP.',

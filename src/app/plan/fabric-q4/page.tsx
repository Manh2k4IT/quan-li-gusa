...
export default function FabricQ4PlanPage() {
  ...
  const [draft, setDraft] = useState({
    ...
    title: 'Báo cáo kế hoạch kho vải Q4',
  ...
  useEffect(() => {
    fetch('/api/plan-assignments')
      ...
      .then((payload: { assignments?: AssignmentApiItem[] }) => setRows((payload.assignments ?? []).filter((item) => item.category === 'Kho vải Quận 4').map((item) => ({ id: item.id, sale: item.assignee?.name ?? item.assignee?.email ?? 'Sale', date: item.date ?? '', status: item.status ?? 'Đang thực hiện', note: item.note ?? '', attachment: item.attachmentName ?? '', sent: true }))))
      .catch(() => setRows([]));
  ...
  const saveToSalesPlan = (saleName: string, status: TaskRow['status'], note: string) => {
    try {
      ...
      const next = [
        {
          id: createPlanId(saleName, 'Kho vải Quận 4'),
        ...
        ...Array.isArray(existing) ? existing.filter((item) => !(item.salesperson === saleName && item.category === 'Kho vải Quận 4')) : [],
      ];
  ...
  const handleOpenAssign = (saleName: string) => {
    ...
    setDraft({
      ...
      manager: 'Quản lý A',
      category: 'Kho vải Quận 4',
      title: matched?.note ? `${matched.note}` : 'Báo cáo kế hoạch kho vải Q4',
      status: 'Đang thực hiện',
      note: matched?.note ?? '',
    ...
    <main className="page-shell">
      <aside className="sidebar">
        ...
        <nav className="nav">
          ...
          <div style={{ margin: '8px 0 0', paddingLeft: '12px', display: 'flex', flexDirection: 'column', gap: '6px' }}>
            ...
            <Link href="/plan/fabric-q4" className="nav-item" style={{ padding: '8px 10px', borderRadius: '10px', background: 'rgba(92, 132, 180, 0.08)', border: '1px solid rgba(145, 175, 215, 0.15)' }}>
              <span>▣</span>
              Báo cáo kế hoạch kho vải Quận 4
            </Link>
            <Link href="/plan/fabric-ben-thanh" className="nav-item" style={{ padding: '8px 10px', borderRadius: '10px' }}>
              <span>▣</span>
      ...
      <section className="content">
        <header className="topbar">
          <div>
            <p className="eyebrow">KHO VẢI Q4</p>
            <h2 style={{ margin: '6px 0 0' }}>Báo cáo kế hoạch kho vải Quận 4</h2>
          </div>

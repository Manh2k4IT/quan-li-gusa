...
export default function FabricBenThanhPlanPage() {
  ...
  useEffect(() => {
    fetch('/api/plan-assignments')
      ...
      .then((payload: { assignments?: AssignmentApiItem[] }) => setRows((payload.assignments ?? []).filter((item) => item.category === 'Kho vải Bến Thành').map((item) => ({ id: item.id, sale: item.assignee?.name ?? item.assignee?.email ?? 'Sale', date: item.date ?? '', status: item.status ?? 'Đang thực hiện', note: item.note ?? '', attachment: item.attachmentName ?? '', sent: true }))))
      .catch(() => setRows([]));
  ...
  const saveToSalesPlan = (saleName: string, status: TaskRow['status'], note: string) => {
    try {
      ...
      const next = [
        {
          id: createPlanId(saleName, 'Kho vải Bến Thành'),
        ...
        ...Array.isArray(existing) ? existing.filter((item) => !(item.salesperson === saleName && item.category === 'Kho vải Bến Thành')) : [],
      ];

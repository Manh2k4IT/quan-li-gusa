...
export default function FashionPlanPage() {
  ...
  const [draft, setDraft] = useState({
    ...
    title: 'Báo cáo kế hoạch thời trang Q4',
  ...
  const filteredRows = useMemo(() => rows.filter((row) => (
    (!saleFilter || row.sale.toLowerCase().includes(saleFilter.toLowerCase()))
    && (!statusFilter || row.status === statusFilter)
    && (!dateFilter || row.date === dateFilter)
  )), [dateFilter, rows, saleFilter, statusFilter]);
  ...
  const handleOpenAssign = (saleName: string) => {
    ...
    setDraft({
      ...
      manager: 'Quản lý A',
      category: 'Thời trang Quận 4',
      title: matched?.note ? `${matched.note}` : 'Báo cáo kế hoạch thời trang Q4',
      status: 'Đang thực hiện',
      note: matched?.note ?? '',
  ...
  const handleSend = async () => {
    ...
    setRows((previous) => [{ id: payload.assignment?.id ?? crypto.randomUUID(), sale: selectedSale, date: draft.date, category: draft.category, status: draft.status, note: draft.note, attachment: draft.attachment, sent: true }, ...previous]);
  ...
  const analyzeReport = async () => {
    ...
    setIsAnalyzing(true);
    setAiAnalysis('Đang phân tích báo cáo...');
    try {
      const reportContent = selectedReport.note.trim();
      const response = await fetch('/api/chat', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ message: `Bạn là người kiểm tra báo cáo Sale. Hãy phân tích trực tiếp nội dung báo cáo nằm giữa hai dấu bắt đầu/kết thúc, không trả lời chung chung và không bỏ qua nội dung này.\n\nBÁO CÁO SALE: ${selectedReport.sale}\nNgày: ${selectedReport.date}\n\n--- BẮT ĐẦU NỘI DUNG BÁO CÁO ---\n${reportContent || 'Sale chưa nhập nội dung báo cáo.'}\n--- KẾT THÚC NỘI DUNG BÁO CÁO ---\n\nHãy chỉ ra báo cáo đang nói gì, vấn đề/rủi ro cụ thể trong nội dung, mức độ đầy đủ và đề xuất xử lý thực tế.` }) });
      const payload = await response.json();
      setAiAnalysis(response.ok ? payload.reply : (payload.message ?? 'Không thể phân tích báo cáo.'));
    } catch { setAiAnalysis('Không thể kết nối AI để phân tích báo cáo.'); }
    ...
    <main className="page-shell">
      <aside className="sidebar">
        ...
        <nav className="nav">
          ...
          <div style={{ margin: '8px 0 0', paddingLeft: '12px', display: 'flex', flexDirection: 'column', gap: '6px' }}>
            <Link href="/plan/fashion-q4" className="nav-item" style={{ padding: '8px 10px', borderRadius: '10px', background: 'rgba(92, 132, 180, 0.08)', border: '1px solid rgba(145, 175, 215, 0.15)' }}>
              <span>▣</span>
              Báo cáo kế hoạch Sale thời trang-kho vải Quận 4-Bến Thành
            </Link>
          </div>
      ...
      <section className="content">
        <header className="topbar">
          <div>
            <p className="eyebrow">THỜI TRANG Q4</p>
            <h2 style={{ margin: '6px 0 0' }}>Báo cáo kế hoạch thời trang Quận 4</h2>
          </div>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'flex-end', gap: '10px', marginBottom: '12px', padding: '12px 14px', background: 'rgba(11,26,38,0.72)', border: '1px solid rgba(115,133,163,0.22)', borderRadius: '12px' }}>
          <label style={{ display: 'flex', flexDirection: 'column', gap: '5px', color: '#b9d8f8', fontSize: '11px', fontWeight: 700 }}>Tên sale<input value={saleFilter} onChange={(event) => setSaleFilter(event.target.value)} placeholder="Tìm tên sale" style={{ width: '180px', padding: '8px 10px', borderRadius: '8px', border: '1px solid rgba(158,176,211,0.25)', background: 'rgba(15,26,38,0.8)', color: '#edf5ff' }} /></label>
          <label style={{ display: 'flex', flexDirection: 'column', gap: '5px', color: '#b9d8f8', fontSize: '11px', fontWeight: 700 }}>Trạng thái<select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)} style={{ width: '170px', padding: '8px 10px', borderRadius: '8px', border: '1px solid rgba(158,176,211,0.25)', background: 'rgba(15,26,38,0.8)', color: '#edf5ff' }}><option value="">Tất cả trạng thái</option><option>Đang thực hiện</option><option>Chờ xác nhận</option><option>Đã duyệt</option><option>Không duyệt</option></select></label>
          <label style={{ display: 'flex', flexDirection: 'column', gap: '5px', color: '#b9d8f8', fontSize: '11px', fontWeight: 700 }}>Ngày<input type="date" value={dateFilter} onChange={(event) => setDateFilter(event.target.value)} style={{ width: '150px', padding: '8px 10px', borderRadius: '8px', border: '1px solid rgba(158,176,211,0.25)', background: 'rgba(15,26,38,0.8)', color: '#edf5ff' }} /></label>
          <button type="button" onClick={() => { setSaleFilter(''); setStatusFilter(''); setDateFilter(''); }} className="ghost-btn" style={{ padding: '8px 12px', fontSize: '11px' }}>Xóa lọc</button>
          <span style={{ marginLeft: 'auto', color: '#8db7da', fontSize: '11px' }}>{filteredRows.length} kết quả</span>
        </div>

        <div style={{ background: 'rgba(11,26,38,0.88)', border: '1px solid rgba(115,133,163,0.28)', borderRadius: '18px', overflow: 'hidden' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 0.9fr 1.3fr 1fr 1.8fr 1.1fr 1.5fr', gap: '10px', padding: '12px 14px', borderBottom: '1px solid rgba(158,176,211,0.18)', color: '#edf5ff', fontWeight: 700, fontSize: '0.7rem' }}>
            <div>Sale</div>
            <div>Ngày</div>
            <div>Phân loại</div>
            <div>Trạng thái</div>
          ...
          {filteredRows.map((row) => (
            <div key={row.id} style={{ display: 'grid', gridTemplateColumns: '1.2fr 0.9fr 1.3fr 1fr 1.8fr 1.1fr 1.5fr', gap: '10px', padding: '12px 14px', borderBottom: '1px solid rgba(158,176,211,0.12)', alignItems: 'center', fontSize: '0.84rem' }}>
              <div style={{ color: '#edf5ff', fontWeight: 700 }}>{row.sale}</div>
              <div style={{ color: '#edf5ff' }}>{row.date}</div>
              <div style={{ color: '#b9d8f8', fontWeight: 700, fontSize: '0.78rem' }}>{row.category}</div>
              <div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(2, 8, 16, 0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 60 }} onClick={() => setSelectedReport(null)}>
          <div onClick={(event) => event.stopPropagation()} style={{ width: 'min(760px, calc(100vw - 32px))', maxHeight: '88vh', overflowY: 'auto', overflowX: 'hidden', boxSizing: 'border-box', background: 'rgba(15, 27, 38, 0.98)', border: '1px solid rgba(140, 160, 190, 0.25)', borderRadius: '18px', padding: '22px 20px', boxShadow: '0 24px 45px rgba(0,0,0,0.28)' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '18px' }}>
              <div><p className="eyebrow" style={{ marginBottom: '6px' }}>SALE REPORT</p><h3 style={{ margin: 0, color: '#edf5ff', fontSize: '1.7rem' }}>Báo cáo của {selectedReport.sale}</h3></div>
              <button type="button" onClick={() => setSelectedReport(null)} style={{ background: 'transparent', border: '1px solid rgba(158, 176, 211, 0.25)', color: '#edf5ff', width: '32px', height: '32px', borderRadius: '8px', cursor: 'pointer' }}>×</button>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
              <div><small style={{ color: '#8db7da' }}>Sale</small><p style={{ margin: '6px 0 0', color: '#edf5ff', fontWeight: 700 }}>{selectedReport.sale}</p></div>
              <div><small style={{ color: '#8db7da' }}>Ngày báo cáo</small><p style={{ margin: '6px 0 0', color: '#edf5ff', fontWeight: 700 }}>{selectedReport.date}</p></div>
              <div><small style={{ color: '#8db7da' }}>Trạng thái</small><p style={{ margin: '6px 0 0', color: '#ffcb6b', fontWeight: 700 }}>{selectedReport.status}</p></div>
              <div><small style={{ color: '#8db7da' }}>File</small><p style={{ margin: '6px 0 0', color: '#9ad7ff', fontWeight: 700 }}>{selectedReport.attachment || 'Không có file'}</p></div>
            </div>
